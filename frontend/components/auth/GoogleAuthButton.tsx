"use client";

import { useCallback, useEffect, useRef, useState } from "react";
import { Button } from "@/components/ui/Button";
import { getGoogleClientId } from "@/lib/env";

type GoogleAuthButtonProps = {
  onCredential: (idToken: string) => Promise<void>;
  disabled?: boolean;
};

type GoogleCredentialResponse = {
  credential?: string;
};

type GoogleAccountsId = {
  initialize: (config: {
    client_id: string;
    callback: (response: GoogleCredentialResponse) => void;
    ux_mode?: "popup";
  }) => void;
  prompt: (callback?: (notification: { isNotDisplayed: () => boolean }) => void) => void;
};

declare global {
  interface Window {
    google?: {
      accounts: {
        id: GoogleAccountsId;
      };
    };
  }
}

const GIS_SRC = "https://accounts.google.com/gsi/client";

function loadGisScript(): Promise<void> {
  const existing = document.querySelector<HTMLScriptElement>(
    `script[src="${GIS_SRC}"]`,
  );

  if (existing) {
    if (window.google?.accounts?.id) return Promise.resolve();
    return new Promise((resolve, reject) => {
      existing.addEventListener("load", () => resolve(), { once: true });
      existing.addEventListener("error", () => reject(new Error("Failed to load Google script.")), {
        once: true,
      });
    });
  }

  return new Promise((resolve, reject) => {
    const script = document.createElement("script");
    script.src = GIS_SRC;
    script.async = true;
    script.defer = true;
    script.onload = () => resolve();
    script.onerror = () => reject(new Error("Failed to load Google script."));
    document.head.appendChild(script);
  });
}

export function GoogleAuthButton({
  onCredential,
  disabled = false,
}: GoogleAuthButtonProps) {
  const clientId = getGoogleClientId();
  const [busy, setBusy] = useState(false);
  const [help, setHelp] = useState<string | null>(
    clientId ? null : "Set NEXT_PUBLIC_GOOGLE_CLIENT_ID in .env.local to enable Google sign-in.",
  );
  const readyRef = useRef(false);

  useEffect(() => {
    if (!clientId) return;

    let cancelled = false;

    (async () => {
      try {
        await loadGisScript();
        if (cancelled || !window.google?.accounts?.id) return;

        window.google.accounts.id.initialize({
          client_id: clientId,
          ux_mode: "popup",
          callback: async (response) => {
            if (!response.credential) {
              setHelp("Google did not return a credential. Try again.");
              return;
            }

            setBusy(true);
            setHelp(null);
            try {
              await onCredential(response.credential);
            } catch (error) {
              setHelp(
                error instanceof Error
                  ? error.message
                  : "Google sign-in failed.",
              );
            } finally {
              setBusy(false);
            }
          },
        });

        readyRef.current = true;
      } catch {
        if (!cancelled) {
          setHelp("Could not load Google Identity Services.");
        }
      }
    })();

    return () => {
      cancelled = true;
    };
  }, [clientId, onCredential]);

  const handleClick = useCallback(() => {
    if (!clientId || !readyRef.current || !window.google?.accounts?.id) {
      setHelp(
        clientId
          ? "Google sign-in is still loading. Try again in a moment."
          : "Set NEXT_PUBLIC_GOOGLE_CLIENT_ID in .env.local to enable Google sign-in.",
      );
      return;
    }

    setHelp(null);
    window.google.accounts.id.prompt((notification) => {
      if (notification.isNotDisplayed()) {
        setHelp(
          "Google prompt was blocked. Check that this origin is allowed in Google Cloud Console.",
        );
      }
    });
  }, [clientId]);

  return (
    <div className="flex flex-col gap-1.5">
      <Button
        type="button"
        variant="secondary"
        className="w-full"
        loading={busy}
        disabled={disabled || !clientId}
        onClick={handleClick}
      >
        <GoogleMark />
        Continue with Google
      </Button>
      {help ? <p className="text-center text-[11px] leading-snug text-ink/55">{help}</p> : null}
    </div>
  );
}

function GoogleMark() {
  return (
    <svg
      aria-hidden
      width="18"
      height="18"
      viewBox="0 0 48 48"
      className="inline-block h-[18px] w-[18px] shrink-0"
    >
      <path
        fill="#FFC107"
        d="M43.6 20.5H42V20H24v8h11.3C33.7 32.7 29.3 36 24 36c-6.6 0-12-5.4-12-12s5.4-12 12-12c3.1 0 5.8 1.2 7.9 3.1l5.7-5.7C34.2 6.1 29.4 4 24 4 12.9 4 4 12.9 4 24s8.9 20 20 20 20-8.9 20-20c0-1.3-.1-2.5-.4-3.5z"
      />
      <path
        fill="#FF3D00"
        d="M6.3 14.7l6.6 4.8C14.7 16 19 13 24 13c3.1 0 5.8 1.2 7.9 3.1l5.7-5.7C34.2 6.1 29.4 4 24 4 16.3 4 9.6 8.3 6.3 14.7z"
      />
      <path
        fill="#4CAF50"
        d="M24 44c5.2 0 10-2 13.6-5.2l-6.3-5.3C29.3 35.3 26.8 36 24 36c-5.3 0-9.7-3.3-11.3-8l-6.5 5C9.5 39.6 16.2 44 24 44z"
      />
      <path
        fill="#1976D2"
        d="M43.6 20.5H42V20H24v8h11.3c-.8 2.2-2.2 4.1-4.1 5.5l.1.1 6.3 5.3C39.2 36.9 44 32 44 24c0-1.3-.1-2.5-.4-3.5z"
      />
    </svg>
  );
}
